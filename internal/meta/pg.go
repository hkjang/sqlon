...
var migrations = []string{
	`CREATE SCHEMA IF NOT EXISTS sqlon_meta`,
	`DO $$
	DECLARE
		mapping TEXT[][] := ARRAY[
			ARRAY['jamypg_users', 'users'],
			ARRAY['jamypg_sessions', 'sessions'],
			ARRAY['jamypg_mcp_keys', 'mcp_keys'],
			ARRAY['jamypg_db_profiles', 'database_profiles'],
			ARRAY['jamypg_profile_grants', 'profile_grants'],
			ARRAY['jamypg_settings', 'settings'],
			ARRAY['jamypg_datasets', 'datasets'],
			ARRAY['jamypg_mcp_activity', 'mcp_activity']
		];
		item TEXT[];
	BEGIN
		FOREACH item SLICE 1 IN ARRAY mapping LOOP
			IF to_regclass(format('public.%I', item[1])) IS NOT NULL
				AND to_regclass(format('sqlon_meta.%I', item[2])) IS NOT NULL THEN
				RAISE EXCEPTION 'ambiguous SQLON meta migration: both public.% and sqlon_meta.% exist', item[1], item[2];
			ELSIF to_regclass(format('public.%I', item[1])) IS NOT NULL THEN
				EXECUTE format('ALTER TABLE public.%I SET SCHEMA sqlon_meta', item[1]);
				EXECUTE format('ALTER TABLE sqlon_meta.%I RENAME TO %I', item[1], item[2]);
			END IF;
		END LOOP;
	END $$`,
	`CREATE TABLE IF NOT EXISTS sqlon_meta.users (
		id TEXT PRIMARY KEY,
		username TEXT NOT NULL,
		display_name TEXT NOT NULL DEFAULT '',
		email TEXT NOT NULL DEFAULT '',
		password_hash TEXT,
		role TEXT NOT NULL DEFAULT 'user' CONSTRAINT users_role_check CHECK (role IN ('admin','dba','user')),
	...
	BEGIN
		IF EXISTS (
			...
			WHERE n.nspname='sqlon_meta' AND t.relname='users' AND c.contype='c'
				AND pg_get_constraintdef(c.oid) LIKE '%role%' AND pg_get_constraintdef(c.oid) NOT LIKE '%dba%'
		) THEN
			ALTER TABLE sqlon_meta.users DROP CONSTRAINT IF EXISTS jamypg_users_role_check;
			ALTER TABLE sqlon_meta.users DROP CONSTRAINT IF EXISTS users_role_check;
			ALTER TABLE sqlon_meta.users ADD CONSTRAINT users_role_check CHECK (role IN ('admin','dba','user'));
		END IF;
	END $$`,
	`CREATE UNIQUE INDEX IF NOT EXISTS sqlon_users_username_key ON sqlon_meta.users (lower(username))`,
	`CREATE UNIQUE INDEX IF NOT EXISTS sqlon_users_provider_subject_key
		ON sqlon_meta.users (provider, provider_subject) WHERE provider_subject IS NOT NULL AND provider_subject <> ''`,
	`CREATE TABLE IF NOT EXISTS sqlon_meta.sessions (
		token_hash TEXT PRIMARY KEY,
		user_id TEXT NOT NULL REFERENCES sqlon_meta.users(id) ON DELETE CASCADE,
		created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
	...
	`CREATE INDEX IF NOT EXISTS sqlon_sessions_user_idx ON sqlon_meta.sessions (user_id)`,
	`CREATE TABLE IF NOT EXISTS sqlon_meta.mcp_keys (
		id TEXT PRIMARY KEY,
		user_id TEXT NOT NULL REFERENCES sqlon_meta.users(id) ON DELETE CASCADE,
	...
	`CREATE INDEX IF NOT EXISTS sqlon_mcp_keys_user_idx ON sqlon_meta.mcp_keys (user_id)`,
	`CREATE TABLE IF NOT EXISTS sqlon_meta.database_profiles (
		id TEXT PRIMARY KEY,
		owner_id TEXT NOT NULL REFERENCES sqlon_meta.users(id),
		definition JSONB NOT NULL,
		visibility TEXT NOT NULL DEFAULT 'private' CHECK (visibility IN ('private','shared')),
		created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
		updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
	)`,
	`CREATE TABLE IF NOT EXISTS sqlon_meta.profile_grants (
		profile_id TEXT NOT NULL REFERENCES sqlon_meta.database_profiles(id) ON DELETE CASCADE,
		user_id TEXT NOT NULL REFERENCES sqlon_meta.users(id) ON DELETE CASCADE,
	...
	`CREATE TABLE IF NOT EXISTS sqlon_meta.settings (
		key TEXT PRIMARY KEY,
		value TEXT NOT NULL DEFAULT '',
		updated_by TEXT NOT NULL DEFAULT '',
		updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
	)`,
	`CREATE TABLE IF NOT EXISTS sqlon_meta.datasets (
		name TEXT PRIMARY KEY,
		content BYTEA NOT NULL,
		updated_by TEXT NOT NULL DEFAULT '',
		updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
	)`,
	`CREATE TABLE IF NOT EXISTS sqlon_meta.mcp_activity (
		id TEXT PRIMARY KEY,
		created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
	...
	`CREATE INDEX IF NOT EXISTS sqlon_mcp_activity_user_idx ON sqlon_meta.mcp_activity (user_id, created_at DESC)`,
	`CREATE INDEX IF NOT EXISTS sqlon_mcp_activity_time_idx ON sqlon_meta.mcp_activity (created_at DESC)`,
	`CREATE INDEX IF NOT EXISTS sqlon_mcp_activity_session_idx ON sqlon_meta.mcp_activity (session_id, created_at DESC)`,
}
...
func (p *PGStore) CreateUser(ctx context.Context, u *User) error {
	_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.users
...
func (p *PGStore) GetUserByID(ctx context.Context, id string) (*User, error) {
	return scanUser(p.db.QueryRowContext(ctx, `SELECT `+userCols+` FROM sqlon_meta.users WHERE id=$1`, id))
}

func (p *PGStore) GetUserByUsername(ctx context.Context, username string) (*User, error) {
	return scanUser(p.db.QueryRowContext(ctx,
		`SELECT `+userCols+` FROM sqlon_meta.users WHERE lower(username)=lower($1)`, username))
}

func (p *PGStore) GetUserByProviderSubject(ctx context.Context, provider, subject string) (*User, error) {
	return scanUser(p.db.QueryRowContext(ctx,
		`SELECT `+userCols+` FROM sqlon_meta.users WHERE provider=$1 AND provider_subject=$2`, provider, subject))
}

func (p *PGStore) ListUsers(ctx context.Context) ([]*User, error) {
	rows, err := p.db.QueryContext(ctx, `SELECT `+userCols+` FROM sqlon_meta.users ORDER BY username`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PGStore) UpdateUser(ctx context.Context, u *User) error {
	...
	res, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.users SET
...
func (p *PGStore) CountUsers(ctx context.Context) (int, error) {
	var n int
	err := p.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM sqlon_meta.users`).Scan(&n)
	return n, err
}

func (p *PGStore) TouchLogin(ctx context.Context, userID string, at time.Time) error {
	_, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.users SET last_login_at=$2 WHERE id=$1`, userID, at)
	return err
}
...
func (p *PGStore) CreateSession(ctx context.Context, s *Session) error {
	_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.sessions
...
func (p *PGStore) GetSession(ctx context.Context, tokenHash string) (*Session, error) {
	...
	err := p.db.QueryRowContext(ctx, `SELECT token_hash, user_id, created_at, expires_at, ip, user_agent, revoked_at
		FROM sqlon_meta.sessions WHERE token_hash=$1`, tokenHash).
...
func (p *PGStore) ExtendSession(ctx context.Context, tokenHash string, until time.Time) error {
	_, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.sessions SET expires_at=$2 WHERE token_hash=$1`, tokenHash, until)
	return err
}

func (p *PGStore) RevokeSession(ctx context.Context, tokenHash string) error {
	_, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.sessions SET revoked_at=now() WHERE token_hash=$1 AND revoked_at IS NULL`, tokenHash)
	return err
}

func (p *PGStore) RevokeUserSessions(ctx context.Context, userID string) error {
	_, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.sessions SET revoked_at=now() WHERE user_id=$1 AND revoked_at IS NULL`, userID)
	return err
}
...
func (p *PGStore) CreateKey(ctx context.Context, k *MCPKey) error {
	_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.mcp_keys
...
func (p *PGStore) GetKeyByHash(ctx context.Context, keyHash string) (*MCPKey, error) {
	return scanKey(p.db.QueryRowContext(ctx, `SELECT `+keyCols+` FROM sqlon_meta.mcp_keys WHERE key_hash=$1`, keyHash))
}

func (p *PGStore) GetKeyByID(ctx context.Context, id string) (*MCPKey, error) {
	return scanKey(p.db.QueryRowContext(ctx, `SELECT `+keyCols+` FROM sqlon_meta.mcp_keys WHERE id=$1`, id))
}

func (p *PGStore) ListKeys(ctx context.Context, userID string) ([]*MCPKey, error) {
	q := `SELECT ` + keyCols + ` FROM sqlon_meta.mcp_keys`
	args := []any{}
	if userID != "" {
		q += ` WHERE user_id=$1`
...
func (p *PGStore) RevokeKey(ctx context.Context, id string) error {
	res, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.mcp_keys SET revoked_at=now() WHERE id=$1 AND revoked_at IS NULL`, id)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (p *PGStore) TouchKey(ctx context.Context, id string, at time.Time) error {
	_, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.mcp_keys SET last_used_at=$2 WHERE id=$1`, id, at)
	return err
}
...
func (p *PGStore) ListProfiles(ctx context.Context) ([]*ProfileRecord, error) {
	rows, err := p.db.QueryContext(ctx,
		`SELECT id, owner_id, definition, visibility, created_at, updated_at FROM sqlon_meta.database_profiles ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PGStore) GetProfile(ctx context.Context, id string) (*ProfileRecord, error) {
	...
	err := p.db.QueryRowContext(ctx,
		`SELECT id, owner_id, definition, visibility, created_at, updated_at FROM sqlon_meta.database_profiles WHERE id=$1`, id).
...
func (p *PGStore) UpsertProfile(ctx context.Context, rec *ProfileRecord, create bool) error {
	if create {
		_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.database_profiles (id, owner_id, definition, visibility)
	...
	res, err := p.db.ExecContext(ctx, `UPDATE sqlon_meta.database_profiles
...
func (p *PGStore) DeleteProfile(ctx context.Context, id string) error {
	res, err := p.db.ExecContext(ctx, `DELETE FROM sqlon_meta.database_profiles WHERE id=$1`, id)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (p *PGStore) ListGrants(ctx context.Context, profileID string) ([]Grant, error) {
	rows, err := p.db.QueryContext(ctx, `SELECT profile_id, user_id, permission, granted_by, granted_at
		FROM sqlon_meta.profile_grants WHERE profile_id=$1 ORDER BY user_id`, profileID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PGStore) ListGrantsForUser(ctx context.Context, userID string) ([]Grant, error) {
	rows, err := p.db.QueryContext(ctx, `SELECT profile_id, user_id, permission, granted_by, granted_at
		FROM sqlon_meta.profile_grants WHERE user_id=$1`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PGStore) SetGrant(ctx context.Context, g Grant) error {
	_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.profile_grants (profile_id, user_id, permission, granted_by)
		VALUES ($1,$2,$3,$4)
...
func (p *PGStore) RemoveGrant(ctx context.Context, profileID, userID string) error {
	res, err := p.db.ExecContext(ctx, `DELETE FROM sqlon_meta.profile_grants WHERE profile_id=$1 AND user_id=$2`, profileID, userID)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (p *PGStore) GetSettings(ctx context.Context) (map[string]string, error) {
	rows, err := p.db.QueryContext(ctx, `SELECT key, value FROM sqlon_meta.settings`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PGStore) SetSetting(ctx context.Context, key, value, updatedBy string) error {
	_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.settings (key, value, updated_by)
		VALUES ($1,$2,$3)
...
func (p *PGStore) DeleteSetting(ctx context.Context, key string) error {
	_, err := p.db.ExecContext(ctx, `DELETE FROM sqlon_meta.settings WHERE key=$1`, key)
	return err
}
...
func (p *PGStore) GetDataset(ctx context.Context, name string) (*DatasetRow, error) {
	var d DatasetRow
	err := p.db.QueryRowContext(ctx, `SELECT name, content, updated_by, updated_at FROM sqlon_meta.datasets WHERE name=$1`, name).
...
func (p *PGStore) ListDatasets(ctx context.Context) ([]*DatasetRow, error) {
	rows, err := p.db.QueryContext(ctx, `SELECT name, content, updated_by, updated_at FROM sqlon_meta.datasets ORDER BY name`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (p *PGStore) PutDataset(ctx context.Context, name string, content []byte, updatedBy string) error {
	_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.datasets (name, content, updated_by)
		VALUES ($1,$2,$3)
...
func (p *PGStore) DeleteDataset(ctx context.Context, name string) error {
	_, err := p.db.ExecContext(ctx, `DELETE FROM sqlon_meta.datasets WHERE name=$1`, name)
	return err
}
...
func (p *PGStore) RecordActivity(ctx context.Context, a *MCPActivity) error {
	...
	_, err := p.db.ExecContext(ctx, `INSERT INTO sqlon_meta.mcp_activity
...
func (p *PGStore) LastPromptForSession(ctx context.Context, sessionID, userID string) (string, error) {
	var prompt string
	err := p.db.QueryRowContext(ctx, `SELECT prompt FROM sqlon_meta.mcp_activity
