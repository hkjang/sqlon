...
func (s *Server) registerAdmin(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/observability/backup", func(w http.ResponseWriter, r *http.Request) {
		if _, ok := s.requireQueryActor(w, r); !ok {
			return
		}
		profiles, ctx, ok := s.fleetProfilesForRequest(w, r)
	...
	mux.HandleFunc("GET /api/observability/config-drift", func(w http.ResponseWriter, r *http.Request) {
		if _, ok := s.requireQueryActor(w, r); !ok {
			return
		}
		profiles, ctx, ok := s.fleetProfilesForRequest(w, r)
