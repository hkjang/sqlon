...
(function () {
  ...
  var GUIDES = {
    ...
    settings: '## 서버 설정 (관리자)\n\n마스터 토큰·허용 Origin·Keycloak SSO·메일 알림을 메타 DB에 저장하고 즉시 적용합니다.\n\n### 메일 알림 (SMTP)\n기본 꺼짐. `mail.enabled=true` 와 `mail.smtp_host` 만 넣으면 사내 릴레이(25번 포트·인증 없음)로 보냅니다. 보내는 이벤트는 변경 계획 승인 요청·실행 가능, 변경 실행 실패, 1분 넘게 걸린 비동기 쿼리 완료, 예약 동기화 실패·회복이며 종류별로 끌 수 있습니다. 자기가 한 일은 자기에게 보내지 않습니다. 저장한 뒤 **시험 발송**으로 릴레이를 확인하고, 발송 기록에서 무엇이 나갔는지 봅니다(본문은 저장하지 않음).',
  };
