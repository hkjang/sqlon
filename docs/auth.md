...
환경변수도 지원: `SQLON_META_DB`, `SQLON_BOOTSTRAP_ADMIN`,
`SQLON_ADMIN_TOKEN`, `SQLON_OIDC_*`. 기존 `JAMYPG_*` 이름은 한 릴리스
동안 별칭으로 지원합니다.

- 메타 DB는 최초 접속 시 기존 `public.jamypg_*` 테이블을 데이터 손실 없이
  `sqlon_meta` 스키마의 `users`, `sessions`, `mcp_keys`,
  `database_profiles`, `profile_grants`, `settings`, `datasets`,
  `mcp_activity`로 이동합니다. 재기동해도 멱등이며, 양쪽 구조가 동시에 존재하는
  애매한 상태는 자동 병합하지 않고 기동을 중단합니다.
...
| MCP 키(`ssk_...`) | MCP 클라이언트/스크립트의 `/mcp`·`/api` 접근 |
...
-oidc-client-id sqlon \
-oidc-client-secret <secret> \
-oidc-redirect-url https://sqlon.example.com/auth/sso/callback
```
...
| 데이터셋 편집·리로드 | ✅ | ✗ | ✗ |
| 승인된 DBA 변경 실행 | ✅ | ✅ | ✗ |
| 모든 DB 프로파일 조회/사용/관리/삭제 | ✅ | 권한 범위 | 소유·grant·shared 범위만 |
| 모든 MCP 키 조회/발급/폐기 | ✅ | 본인 키만 | 본인 키만 |
...
- 발급 시 원본 키(`ssk_<64hex>`)는 **한 번만** 표시되고 서버에는 SHA-256
  해시만 저장됩니다.
- 기존에 발급한 `jsk_...` 키는 한 릴리스 동안 계속 인증되지만 회전 후에는
  `ssk_...` 키가 발급됩니다.
...
```json
{
  "mcpServers": {
    "sqlon": {
      "url": "https://sqlon.example.com/mcp",
...
`X-MCP-Key: ssk_...` 헤더도 동일하게 동작합니다. stdio 전송은 로컬 신뢰
경계이므로 키가 필요 없습니다.
