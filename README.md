...
| 표준 컨테이너 | `docker build -t sqlon/sqlon:v0.1.5 .` |
...
- MCP endpoint: `http://127.0.0.1:6767/mcp`
- Web admin: `http://127.0.0.1:6767/admin`
- Swagger UI: `http://127.0.0.1:6767/docs`
- Health check: `http://127.0.0.1:6767/healthz`
...
```sh
docker build -t sqlon/sqlon:v0.1.5 .
...
go run ./cmd/sqlon -data data/metadb -addr 127.0.0.1:6767
...
```sh
go run ./cmd/sqlon -data data/sakila -addr 127.0.0.1:6767   # 프로파일: pg-sakila / mysql-sakila / mariadb-sakila
```
...
```powershell
go run ./cmd/sqlon -transport http -data .\data\metadb -addr 127.0.0.1:6767
```
...
```powershell
Invoke-RestMethod http://127.0.0.1:6767/healthz
```
...
```sh
sqlon -transport http -addr 0.0.0.0:6767 \
